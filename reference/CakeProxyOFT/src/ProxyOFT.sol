...
contract CakeProxyOFT is Ownable, Pausable, ProxyOFTWithFee {
    ...
    function fallbackWithdraw(address to, uint256 amount) external onlyOperator {
        innerToken.safeTransfer(to, amount);
        emit FallbackWithdraw(to, amount);
    }

    function dropFailedMessage(uint16 srcChainId, bytes memory srcAddress, uint64 nonce) external onlyOperator {
